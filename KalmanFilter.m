...
H = zeros (5, N);
sig = zeros (5, 5*N); % priori or posteri covariance matrix. K = zeros (5, N); % kalman gain. XX = zeros (5, N); % kalman coefficient for yy. y = zeros (1, N); % requiring signal (desired signal)
...
XX(1:5,k) =(I - K(1:5,k)*F(1:5,k)')*XX(1:5,k-1) + (K(1:5,k)*y(k)); % posteriori value of estimate
X(k)
